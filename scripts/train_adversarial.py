...
from nfts.pipelines.vanilla import train
...
def main():
    ...
    parser.add_argument("--num_warmup_episodes", type=int, default=1000)
    ...
    if args.resume:
        model.load_state_dict(torch.load(f"./models/nfts/{args.backbone}_{args.setting}.pth"), strict=True)
    model.to(args.device)
    train(model, data_loader, args, use_adversarial_loss=True)
