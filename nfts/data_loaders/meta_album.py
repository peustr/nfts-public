...
class MetaAlbumDataset(Dataset):
    def __init__(self, data_path, datasets, img_size=128):
        if len(datasets) == 1:
    ...
    def extract_info(self, dataset):
        info = {
            "img_path": f"{self.data_path}/{dataset}/images/",
            "metadata_path": f"{self.data_path}/{dataset}/labels.csv",
            "label_col": "CATEGORY",
            "file_col": "FILE_NAME",
        }
...
def create_datasets(datasets, data_path):
    torch_datasets = []
    for dataset in datasets:
        torch_dataset = MetaAlbumDataset(data_path, [dataset])
        torch_datasets.append(torch_dataset)
    return torch_datasets


class Task:
    def __init__(self, n_way, k_shot, query_size, data, labels, dataset):
        self.n_way = n_way
        self.k_shot = k_shot
...
class MetaAlbumDataLoaderCrossDomain:
    def __init__(self, datasets, num_tasks, episodes_config, test_loader=False):
        ...
        if not self.test_loader:
            while True:
                ...
                for _ in range(self.num_tasks):
                    ...
                    n_way, k_shot = self.prepare_task_config(num_classes, random_gen)
                    ...
                    for c in classes:
                        idx = idx_per_label[c]
                        selected_idx = random_gen.choice(idx, total_examples, replace=False)
                    ...
                    # Return the task
                    task = Task(n_way, k_shot, self.query_size, data, labels, dataset.name)
                    yield task
        else:
...
class MetaAlbumEpisodicDataLoader:
    def __init__(self, data_path, datasets, num_episodes, episodes_config):
        datasets = create_datasets(datasets.split(","), data_path)
        self._data_loader = MetaAlbumDataLoaderCrossDomain(datasets, num_episodes, episodes_config)
        self.generator = iter(self._data_loader.generator(0))
    ...
    def __next__(self):
        ...
        return data[:support_size], labels[:support_size], data[support_size:], labels[support_size:]
